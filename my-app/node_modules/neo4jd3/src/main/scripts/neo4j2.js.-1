...
wordRegexp = function(words) {
    ...
    return tokenBase = function(stream, state) {
    ...
    }, indentUnit = config.indentUnit, curPunc = void 0, funcs = wordRegexp(["str", "min", "labels", "max", "type", "lower", "upper", "length", "type", "id", "coalesce", "head", "last", "nodes", "relationships", "extract", "filter", "tail", "range", "reduce", "abs", "round", "sqrt", "sign", "replace", "substring", "left", "right", "ltrim", "rtrim", "trim", "collect", "distinct", "split", "toInt", "toFloat"]), preds = wordRegexp(["all", "any", "none", "single", "not", "in", "has", "and", "or", "like", "ilike"]), keywords = wordRegexp(["start", "merge", "load", "csv", "using", "periodic commit", "on create", "on match", "match", "index on", "drop", "where", "with", "limit", "skip", "order", "by", "return", "create", "delete", "set", "unique", "unwind"]), operatorChars = /[*+\-<>=&|~]/, {
        ...
        indent: function(state, textAfter) {
            ...
            return closing = context && firstChar === context.type, context ? "keywords" === context.type ? newlineAndIndent : context.align ? context.col + (closing ? 0 : 1) : context.indent + (closing ? 0 : indentUnit) : 0
        }
    }
...
}, CodeMirror.defineMIME("application/x-cypher-query", "cypher"), "function" != typeof String.prototype.trim && (String.prototype.trim = function() {
...
var baseURL, restAPI;
baseURL = "", restAPI = baseURL + "/db/data", angular.module("neo4jApp.settings", ["neo4jApp.utils"]).constant("Settings", {
    cmdchar: ":",
    endpoint: {
...
angular.module("neo4jApp.controllers", ["neo4jApp.utils"]), angular.module("neo4jApp.directives", ["ui.bootstrap.modal", "neo4jApp.utils"]), angular.module("neo4jApp.filters", []), angular.module("neo4jApp.services", ["LocalStorageModule", "neo4jApp.settings", "neo4jApp.utils", "base64", "AsciiTableModule", "auth0", "firebase", "angular-jwt"]), app = angular.module("neo4jApp", ["ngAnimate", "neo4jApp.controllers", "neo4jApp.directives", "neo4jApp.filters", "neo4jApp.services", "neo4jApp.animations", "neo.exportable", "neo.csv", "ui.bootstrap.dropdown", "ui.bootstrap.position", "ui.bootstrap.tooltip", "ui.bootstrap.popover", "ui.bootstrap.tabs", "ui.bootstrap.carousel", "ui.bootstrap.alert", "ui.codemirror", "ui.sortable", "angularMoment", "ngSanitize"]).config(["authProvider", "localStorageServiceProvider", function(authProvider, localStorageServiceProvider) {
    ...
    return timer = null, $scope.check = function() {
        var ts;
        return $timeout.cancel(timer), ts = (new Date).getTime(), Server.status("?t=" + ts).success(function(r) {
            ...
            return AuthService.isConnected().then(function() {
                ...
                var ref;
                return 401 === (ref = response.status) || 403 === ref || 429 === ref ? ($scope.offline = !1, $scope.unauthorized = !0) : ($scope.offline = !0, $scope.unauthorized = !1)
            }).then(function() {
    ...
    return cmdchar = Settings.cmdchar, topicalize = function(input) {
        ...
        exec: ["ProtocolFactory", function(ProtocolFactory) {
            return function(input, q) {
                return ProtocolFactory.getSchemaService().getSchema(input).then(function(res) {
        ...
        exec: ["Settings", "SettingsStore", function(Settings, SettingsStore) {
            return function(input, q) {
                ...
                return "reset" === argv(input)[1] ? (SettingsStore.reset(), q.resolve(Settings), q.promise) : (matches = /^[^\w]*config\s+([^:]+):?([\S\s]+)?$/.exec(input), null != matches ? (ref = [matches[1], matches[2]], key = ref[0], value = ref[1], null != value ? (value = function() {
                    try {
                        return eval(value)
                    } catch (_error) {}
        ...
        exec: ["Cypher", "CypherGraphModel", "CypherParser", function(Cypher, CypherGraphModel, CypherParser) {
            return function(input, q) {
    ...
    return function(input) {
        return "string" != typeof input ? "" : input.replace(/^(\d\.\d)\.\d((-.+)?)$/, "$1$3")
    }
    ...
    return {
        ...
        link: function(scope, elm, attr) {
            ...
            return emptyMarker = function() {
                ...
                for (k in obj) hasProp.call(obj, k) && (v = obj[k], html += "<tr><th>" + k + "</th><td>" + cell2html(v) + "</td></tr>");
...
angular.module("neo4jApp.directives").directive("generalRequestTable", ["Utils", function(Utils) {
...
}]), angular.module("neo4jApp.controllers").controller("D3GraphCtrl", ["$attrs", "$element", "$parse", "$window", "$rootScope", "$scope", "$interval", "$timeout", "CircularLayout", "GraphExplorer", "GraphStyle", "Editor", function($attrs, $element, $parse, $window, $rootScope, $scope, $interval, $timeout, CircularLayout, GraphExplorer, GraphStyle, Editor) {
    ...
    return graphView = null, this.getGraphView = function() {
        ...
        return $rootScope.stickyDisplayInternalRelationships = displayInternalRelationships, graphView ? displayInternalRelationships ? GraphExplorer.internalRelationships(graphView.graph, [], graphView.graph.nodes()).then(function() {
    ...
    return httpOptions = {
        ...
        return Server.prototype.options = function(path, options) {
            ...
            return null == params && (params = ""), this.options(Settings.endpoint.rest + "/", {
    ...
    return $controllerProvider.register("MainCtrl", ["$rootScope", "$window", "$q", "Server", "Frame", "AuthService", "AuthDataService", "ConnectionStatusService", "Settings", "SettingsStore", "motdService", "UsageDataCollectionService", "Utils", "CurrentUser", "ProtocolFactory", function($scope, $window, $q, Server, Frame, AuthService, AuthDataService, ConnectionStatusService, Settings, SettingsStore, motdService, UDC, Utils, CurrentUser, ProtocolFactory) {
        var executePostConnectCmd, fetchJMX, initailConnect, license, mapServerConfig, onboardingSequence, pickFirstFrame, refreshAllowOutgoingConnections, refreshPolicies, setAndSaveSetting;
        return $scope.CurrentUser = CurrentUser, $scope.ConnectionStatusService = ConnectionStatusService, initailConnect = !0, $scope.kernel = {}, $scope.refresh = function() {
            return $scope.unauthorized || $scope.offline ? "" : ($scope.server = Server.info($scope.server), $scope.host = $window.location.host, $q.when().then(function() {
                return ProtocolFactory.getMetaService().getMeta().then(function(res) {
                    return $scope.labels = res.labels, $scope.relationships = res.relationships, $scope.propertyKeys = res.propertyKeys
                })
            }).then(function() {
                return ProtocolFactory.getVersionService().getVersion($scope.version).then(function(res) {
            ...
            return ProtocolFactory.getJmxService().getJmx(["org.neo4j:instance=kernel#0,name=Configuration", "org.neo4j:instance=kernel#0,name=Kernel", "org.neo4j:instance=kernel#0,name=Store file sizes"]).then(function(response) {
            ...
            edition: "community",
            enterpriseEdition: !1
        }, $scope.neo4j.config = {}, $scope.$on("db:changed:labels", $scope.refresh), $scope.today = Date.now(), $scope.cmdchar = Settings.cmdchar, $scope.goodBrowser = !/msie/.test(navigator.userAgent.toLowerCase()), $scope.$watch("offline", function(serverIsOffline) {
            ...
            return CurrentUser.init(), AuthService.hasValidAuthorization(retainConnection = !0).then(function() {
                return Frame.closeWhere(Settings.cmdchar + "server connect"), Frame.create({
                    input: "" + Settings.initCmd
                }), Settings.onboarding ? onboardingSequence() : void 0
            }, function(r) {
            ...
            return val ? ($scope.neo4j.version = val.version, $scope.neo4j.edition = val.edition, $scope.neo4j.enterpriseEdition = "enterprise" === val.edition, $scope.$emit("db:updated:edition", val.edition), val.version ? $scope.motd.setCallToActionVersion(val.version) : void 0) : ""
        }, !0)
    }])
...
angular.module("neo4jApp.services").factory("Relationship", [function() {
    ...
    return {
        ...
        internalRelationships: function(graph, existingNodes, newNodes) {
            ...
            return q = $q.defer(), 0 === newNodes.length ? (q.resolve(), q.promise) : (newNodeIds = newNodes.map(function(node) {
                return node.id
            }), existingNodeIds = existingNodes.map(function(node) {
                return node.id
            }).concat(newNodeIds), Cypher.transaction().commit("MATCH (a)-[r]-(b) WHERE id(a) IN[" + existingNodeIds.join(",") + "]\nAND id(b) IN[" + newNodeIds.join(",") + "]\nRETURN r;").then(function(_this) {
    ...
    return provider = this, this.defaultStyle = {
        ...
        return GraphStyle.prototype.selector = function(item) {
            ...
            var ref, selector;
            return null == node && (node = {}), selector = this.nodeSelector(node), (null != (ref = node.labels) ? ref.length : void 0) > 0 && this.setDefaultNodeStyling(selector, node), this.calculateStyle(selector)
            ...
            var key, props;
            angular.isObject(data) || (data = provider.defaultStyle), this.rules.length = 0;
            ...
            return ips = str.replace(/\{([^{}]*)\}/g, function(a, b) {
                var r;
                return r = item.propertyMap[b], "object" == typeof r && (r = r.join(", ")), "string" == typeof r || "number" == typeof r ? r : ""
                ...
                var r;
                return r = item[b], "string" == typeof r || "number" == typeof r ? r : ""
            })
        }, GraphStyle
    ...
    return $scope.sizes = graphStyle.defaultSizes(), $scope.arrowWidths = graphStyle.defaultArrayWidths(), $scope.colors = graphStyle.defaultColors(), $scope.currentItem = null, $scope.inspectorContracted = !0, $scope.inspectorChanged = !1, $scope.inspectorFixed = !1, inspectorItem = function(item, type) {
        return {
            data: item,
            type: type,
    ...
    return $scope.graph = null, $scope.sizes = graphStyle.defaultSizes(), $scope.arrowWidths = graphStyle.defaultArrayWidths(), $scope.colors = graphStyle.defaultColors(), $scope.labelsContracted = !0, $scope.typesContracted = !0, graphStats = function(graph) {
        ...
        for (resultLabels = {}, resultRelTypes = {}, stats = {
            ...
            for (node = ref[i], null == (base = stats.labels)[""] && (base[""] = {
                ...
                }), stats.labels[""].count++, ref1 = node.labels, ignored = j = 0, len1 = ref1.length; len1 > j; ignored = ++j) label = ref1[ignored], null == (base1 = stats.labels)[label] && (base1[label] = {
                label: label,
...
angular.module("neo4jApp.controllers").controller("CypherResultCtrl", ["$rootScope", "$scope", "AsciiTableFactory", function($rootScope, $scope, AsciiTableFactory) {
    var asciiTable, getTimeString, ref, tableData;
    return $scope.displayInternalRelationships = null != (ref = $rootScope.stickyDisplayInternalRelationships) ? ref : !0, $scope.availableModes = [], $scope.slider = {
        min: 4,
        max: 20
        ...
        if (resp) return $scope.availableModes = [], resp.errors || ((null != (ref1 = resp.graph) ? ref1._nodes.length : void 0) && $scope.availableModes.push("graph"), null != (null != (ref2 = resp.table) ? ref2.size : void 0) && $scope.availableModes.push("table"), (null != (ref3 = resp.table) ? ref3._response.plan : void 0) && $scope.availableModes.push("plan")), resp.raw && $scope.availableModes.push("raw"), resp.errors && $scope.availableModes.push("errors"), (null != (ref4 = resp.notifications) ? ref4.length : void 0) && $scope.availableModes.push("messages"), (null != (ref5 = resp.table) ? ref5.size : void 0) && $scope.availableModes.push("text"), (null != (ref6 = resp.table) ? ref6._response.data.length : void 0) && (tableData = resp.table._response), $scope.loadAscii(), $scope.tab = $rootScope.stickyTab, $scope.isAvailable("messages") && $scope.frame.showCypherNotification ? $scope.tab = "messages" : $scope.isAvailable("plan") ? $scope.tab = "plan" : $scope.isAvailable("errors") && ($scope.tab = "errors"), null == $scope.tab && ($scope.tab = $scope.availableModes[0] || "table"), $scope.availableModes.indexOf($scope.tab) >= 0 ? void 0 : $scope.tab = "table"
    }), $scope.loadAscii = function() {
        var res, rows;
        if (tableData && tableData.data) return rows = tableData.data.map(function(data_obj) {
            return data_obj.row
        }), rows.splice(0, 0, tableData.columns), res = asciiTable.get(rows, {
            maxColumnWidth: $scope.ascii_col_width
        }), $scope.slider.max = asciiTable.maxWidth, $scope.ascii = res
    }, $scope.$watch("ascii_col_width", function(n, o) {
        return $scope.loadAscii()
    }), $scope.setActive = function(tab) {
        return null == tab && (tab = "graph" === $scope.tab ? "table" : "graph"), $rootScope.stickyTab = $scope.tab = tab
        ...
        return (null != frame ? frame.response : void 0) ? (updatesMessages = [], (null != (ref1 = frame.response.table) && null != (ref2 = ref1._response) && null != (ref3 = ref2.columns) ? ref3.length : void 0) && (updatesMessages = $scope.updatesStatistics(frame)), rowsStatistics = $scope.returnedRowsStatistics(frame), messages = [].concat(updatesMessages, rowsStatistics), $scope.formatStatisticsOutput(messages)) : void 0
    }, $scope.graphStatistics = function(frame) {
        var graph, internalRelationships, message, plural, ref1;
        return null != (null != frame && null != (ref1 = frame.response) ? ref1.graph : void 0) ? (graph = frame.response.graph, plural = function(collection, noun) {
        ...
        return null != (null != frame && null != (ref1 = frame.response) && null != (ref2 = ref1.table) ? ref2._response.plan : void 0) ? (root = frame.response.table._response.plan.root, collectHits = function(operator) {
            var child, hits, i, len, ref3, ref4;
            if (hits = null != (ref3 = operator.DbHits) ? ref3 : 0, operator.children)
                for (ref4 = operator.children, i = 0, len = ref4.length; len > i; i++) child = ref4[i], hits += collectHits(child);
            return hits
        }, message = "Cypher version: " + root.version + ", planner: " + root.planner + ", runtime: " + root.runtime + ".", collectHits(root) && (message += " " + collectHits(root) + " total db hits in " + frame.response.responseTime + " ms."), message) : void 0
        ...
        var messages, ref1;
        return messages = [], null != (null != frame && null != (ref1 = frame.response) ? ref1.table : void 0) && (messages.push("returned " + frame.response.table.size + " " + (1 === frame.response.table.size ? "row" : "rows")), messages = getTimeString(frame, messages, "returnedRows"), frame.response.table.size > frame.response.table.displayedSize && messages.push("displaying first " + frame.response.table.displayedSize + " rows")), messages
    }, $scope.updatesStatistics = function(frame) {
        var field, messages, nonZeroFields, ref1, stats;
        return messages = [], null != (null != frame && null != (ref1 = frame.response) ? ref1.table : void 0) && (stats = frame.response.table.stats, nonZeroFields = $scope.getNonZeroStatisticsFields(frame), messages = function() {
            var i, len, results;
            ...
            return results
        }(), messages = getTimeString(frame, messages, "updates")), messages
    }, $scope.getNonZeroStatisticsFields = function(frame) {
        var field, fields, i, len, nonZeroFields, ref1, stats;
        if (nonZeroFields = [], null != (null != frame && null != (ref1 = frame.response) ? ref1.table : void 0))
        ...
        var ref1;
        if (null != (ref1 = frame.response) ? ref1.responseTime : void 0) return "Request finished in " + frame.response.responseTime + " ms."
        ...
        return timeMessage = " in " + frame.response.responseTime + " ms", "updates" === context && messages.length && !(null != (ref1 = frame.response.table._response.columns) ? ref1.length : void 0) && (messages.push("statement executed"), messages[messages.length - 1] += timeMessage), "returnedRows" === context && ((null != (ref2 = frame.response.table._response.columns) ? ref2.length : void 0) || !(null != (ref3 = frame.response.table._response.columns) ? ref3.length : void 0) && !$scope.getNonZeroStatisticsFields(frame).length) && (messages[messages.length - 1] += timeMessage), messages
...
angular.module("neo4jApp.services").provider("Frame", [function() {
    var self;
    return self = this, this.interpreters = [], this.$get = ["$injector", "$q", "Collection", "Settings", "Utils", function($injector, $q, Collection, Settings, Utils) {
        var Frame, Frames, frames;
        return Frame = function() {
            function Frame(data) {
                null == data && (data = {}), this.getDetailedErrorText = bind(this.getDetailedErrorText, this), this.setError = bind(this.setError, this), this.addErrorText = bind(this.addErrorText, this), this.resetError = bind(this.resetError, this), this.setErrorMessages = bind(this.setErrorMessages, this), this.templateUrl = null, angular.isString(data) ? this.input = data : angular.extend(this, data), null == this.id && (this.id = UUID.genV1().toString())
            }
            ...
            return extend(Frames, superClass), Frames.prototype.create = function(data) {
                ...
                for (null == input && (input = ""), intr = null, input = Utils.stripComments(input.trim()), firstWord = Utils.firstWord(input).toLowerCase(), ref = self.interpreters, j = 0, len = ref.length; len > j; j++)
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            return topic = attrs.helpTopic, command = "help", topic ? element.on("click", function(e) {
                return e.preventDefault(), topic = topic.toLowerCase().trim().replace(/-/g, " "), Frame.create({
...
angular.module("neo4jApp.services").service("AuthService", ["ConnectionStatusService", "ProtocolFactory", "Settings", "$q", function(ConnectionStatusService, ProtocolFactory, Settings, $q) {
    ...
    return setConnectionAuthData = function(username, password, emitChange) {
        ...
        return AuthService.prototype.authenticate = function(username, password) {
            var promise, retainConnection, that, withoutCredentials;
            return that = this, this.current_password = password, setConnectionAuthData(username, password), promise = this.makeRequest(withoutCredentials = !1, retainConnection = !0), promise.then(function(r) {
            ...
            var p, q, retainConnection, withoutCredentials;
            return q = $q.defer(), p = this.makeRequest(withoutCredentials = !0, retainConnection = !0), p.then(function(r) {
                return clearConnectionAuthData(), ConnectionStatusService.setAuthorizationRequired(!1), q.resolve(r)
            }, function(r) {
            ...
            var q, req, that;
            return null == retainConnection && (retainConnection = !1), that = this, q = $q.defer(), req = this.authorizationRequired(), req.then(function(r) {
                return ConnectionStatusService.setConnected(!0), q.resolve(r)
            }, function(r) {
                return ConnectionStatusService.connectionAuthData().length > 0 ? that.isConnected(retainConnection).then(function(r) {
                    return q.resolve(r)
                }, function(r) {
                    return ConnectionStatusService.setConnected(!1), q.reject(r)
                }) : (ConnectionStatusService.setConnected(!1), q.reject(r))
            }), q.promise
        }, AuthService.prototype.isConnected = function(retainConnection) {
            var p, q, withoutCredentials;
            return null == retainConnection && (retainConnection = !1), q = $q.defer(), p = this.makeRequest(withoutCredentials = !1, retainConnection), p.then(function(rr) {
                return ConnectionStatusService.setConnected(!0), q.resolve(rr)
            }, function(rr) {
                return 401 === rr.status && clearConnectionAuthData(), q.reject(rr)
            }), q.promise
        }, AuthService.prototype.makeRequest = function(withoutCredentials, retainConnection) {
            return null == withoutCredentials && (withoutCredentials = !1), null == retainConnection && (retainConnection = !1), ProtocolFactory.getAuthService().makeRequest(withoutCredentials, retainConnection)
        }, AuthService.prototype.forget = function() {
            return this.getCurrentUser() && clearConnectionAuthData(), ConnectionStatusService.setConnected(!1), ProtocolFactory.getAuthService().clearConnection()
        }, AuthService.prototype.setNewPassword = function(old_passwd, new_passwd) {
            var q, that;
            return that = this, q = $q.defer(), setConnectionAuthData(ConnectionStatusService.connectedAsUser(), old_passwd), ProtocolFactory.getAuthService().setNewPassword(ConnectionStatusService.connectedAsUser(), new_passwd).then(function(r) {
                var emitChange;
    ...
    var cached_authorization_data, cached_credential_timeout, cached_retain_connection_credentials;
    return cached_authorization_data = localStorageService.get("authorization_data") || "", cached_retain_connection_credentials = null, cached_credential_timeout = null, this.setAuthData = function(authdata) {
...
}]), angular.module("neo4jApp.controllers").controller("AuthCtrl", ["$scope", "AuthService", "ConnectionStatusService", "Frame", "CurrentUser", "Settings", "Utils", "$timeout", function($scope, AuthService, ConnectionStatusService, Frame, CurrentUser, Settings, Utils, $timeout) {
    var setPolicyMessage;
    return $scope.username = "neo4j", $scope.password = "", $scope.current_password = "", $scope.connection_summary = ConnectionStatusService.getConnectionStatusSummary(), $scope.static_user = $scope.connection_summary.user, $scope.static_host = Utils.getServerHostname(Settings), $scope.static_is_authenticated = $scope.connection_summary.is_connected, $scope.policy_message = "", $scope.CurrentUser = CurrentUser, setPolicyMessage = function() {
        ...
        return $scope.frame.resetError(), $scope.password.length || $scope.frame.addErrorText("You have to enter a password. "), $scope.username.length || $scope.frame.addErrorText("You have to enter a username. "), $scope.frame.getDetailedErrorText().length ? void 0 : AuthService.authenticate($scope.username, $scope.password).then(function(r) {
            return $scope.frame.resetError(), $scope.connection_summary = ConnectionStatusService.getConnectionStatusSummary(), $scope.static_user = $scope.connection_summary.user, $scope.static_is_authenticated = $scope.connection_summary.is_connected, $scope.static_host = Utils.getServerHostname(Settings), setPolicyMessage(), Frame.create({
                input: "" + Settings.initCmd
            }), $scope.focusEditor()
        }, function(r) {
            var ref;
            return 403 === r.status && (null != (ref = r.data.password_change) ? ref.length : void 0) ? ($scope.current_password = $scope.password, $scope.password_change_required = !0) : $scope.frame.setError(r)
        })
    ...
    return AuthService.forget(), $scope.static_user = ConnectionStatusService.connectedAsUser(), $scope.static_is_authenticated = ConnectionStatusService.isConnected(), $scope.focusEditor()
    ...
    return $scope.new_password = "", $scope.new_password2 = "", $scope.current_password = "", $scope.password_changed = !1, $scope.$parent.frame.resetError(), $scope.static_user = ConnectionStatusService.connectedAsUser(), $scope.static_is_authenticated = ConnectionStatusService.isConnected(), $scope.showCurrentPasswordField = function() {
        ...
        return is_authenticated = ConnectionStatusService.isConnected(), $scope.$parent.frame.resetError(), $scope.$parent.password_change_required && ($scope.current_password = $scope.$parent.current_password), $scope.static_user = ConnectionStatusService.connectedAsUser(), $scope.current_password.length || $scope.$parent.frame.addErrorText("You have to enter your current password. "), $scope.new_password.length || $scope.$parent.frame.addErrorText("You have to enter a new password. "), $scope.new_password !== $scope.new_password2 && $scope.$parent.frame.addErrorText("The new passwords mismatch, try again. "), $scope.$parent.frame.getDetailedErrorText().length ? void 0 : AuthService.setNewPassword($scope.current_password, $scope.new_password).then(function() {
            return is_authenticated || ($scope.$parent.defaultPasswordChanged(), Frame.create({
                input: "" + Settings.initCmd
            })), $scope.password_changed = !0, $scope.$parent.frame.resetError(), $scope.focusEditor()
        }, function(r) {
    ...
    return interceptor = {
        request: function(config) {
            ...
            return config.headers["X-Ajax-Browser-Auth"] = !0, isLocalRequest = !0, /^https?:/.test(config.url) && (url = document.location.origin || window.location.protocol + "//" + window.location.host, config.url.indexOf(0 > url) && (isLocalRequest = !1)), config.skipAuthHeader || !isLocalRequest ? config : (header = AuthDataService.getAuthData(), header && (config.headers.Authorization = "Basic " + header), config)
        }
    }
...
NeoD3Geometry = function() {
    ...
    return square = function(distance) {
        ...
        for (template = style.forNode(node).get("caption"), captionText = style.interpolate(template, node), fontFamily = "sans-serif", fontSize = parseFloat(style.forNode(node).get("font-size")), lineHeight = fontSize, measure = function(text) {
                ...
                return baseline = (1 + iLine - lineCount / 2) * lineHeight, constainingHeight = lineCount / 2 > iLine ? baseline - lineHeight : baseline, lineWidth = 2 * Math.sqrt(square(node.radius) - square(constainingHeight)), {
                    node: node,
                    text: "",
                    baseline: baseline,
...
neo.models.Node = function() {
    ...
    return maxChildOperators = 2, maxComparableRows = 1e6, maxComparableDbHits = 1e6, operatorWidth = 180, operatorCornerRadius = 4, operatorHeaderHeight = 18, operatorHeaderFontSize = 11, operatorDetailHeight = 14, maxCostHeight = 50, detailFontSize = 10, operatorMargin = 50, operatorPadding = 3, rankMargin = 50, margin = 10, standardFont = "'Helvetica Neue',Helvetica,Arial,sans-serif", fixedWidthFont = "Monaco,'Courier New',Terminal,monospace", linkColor = "#DFE1E3", costColor = "#F25A29", dividerColor = "#DFE1E3", operatorColors = colorbrewer.Blues[9].slice(2), operatorCategories = {
        ...
        for (details = [], wordWrap = function(string, className) {
                ...
                className: "padding"
            })), (expression = null != (ref2 = null != (ref3 = operator.LegacyExpression) ? ref3 : operator.ExpandExpression) ? ref2 : operator.LabelName) && (wordWrap(expression, "expression"), details.push({
                className: "padding"
...
neo.utils.adjacentAngles = function() {
        ...
        return pairwiseArcsRelationshipRouting.prototype.measureRelationshipCaption = function(relationship, caption) {
            ...
            for (results = [], j = 0, len = nodePairs.length; len > j; j++)
                if (nodePair = nodePairs[j], nodePair.isLoop()) {
                    for (angles = [], node = nodePair.nodeA, k = 0, len1 = relationships.length; len1 > k; k++) relationship = relationships[k], relationship.isLoop() || (relationship.source === node && angles.push(relationship.naturalAngle), relationship.target === node && angles.push(relationship.naturalAngle + 180));
    ...
    function() {
        ...
        }), neo.renderers.node.push(nodeOutline), neo.renderers.node.push(nodeCaption), neo.renderers.node.push(nodeRing), neo.renderers.relationship.push(arrowPath), neo.renderers.relationship.push(relationshipType), neo.renderers.relationship.push(relationshipOverlay)
    }(),
    function() {
        ...
        return at = AsciiTable, AsciiTableClass = function() {
            ...
            return AsciiTableClass.prototype.get = function(items, options) {
                return null == options && (options = {}), this.maxWidth || (this.maxWidth = at.getMaxColumnWidth(items)), at.run(items, options)
            }, AsciiTableClass
        }(), {
        ...
        }, this.connected_user = "", this.authorization_required = !0, this.is_connected = !1, this.session_start_time = new Date, this.session_countdown = null, this.waiting_policies = !1, this.setConnectionAuthData = function(username, password, emitChange) {
            return null == emitChange && (emitChange = !1), this.setConnectedUser(username), this.updatePersistentAuthData(username + ":" + password), emitChange ? $rootScope.$emit("connection:authdata_updated") : void 0
...
angular.module("neo4jApp.services").service("Intercom", ["$log", "$window", "$timeout", function($log, $window, $timeout) {
    ...
    return $scope.settings = Settings, $scope.save = function() {
        ...
        return Frame.create({
            input: Settings.cmdchar + "help bolt encryption"
        })
    }, $scope.toggleBoltUsage = function() {
        return $rootScope.bolt_connection_failure = !1, Settings.useBolt ? Bolt.connect() : void 0
    ...
    return this.prepareForExport = function($element, dimensions) {
        ...
        }), svg.selectAll(".overlay, .ring").remove(), svg.selectAll(".context-menu-item").remove(), svg.selectAll("text").attr("font-family", "sans-serif"), svg.attr("width", dimensions.width), svg.attr("height", dimensions.height), svg.attr("viewBox", dimensions.viewBox), svg
    }, this
}]);
var bind = function(fn, me) {
        return function() {
...
angular.module("neo4jApp.services").service("CurrentUser", ["Settings", "Editor", "AuthService", "NTN", "localStorageService", "AuthDataService", "jwtHelper", "$q", "$rootScope", "UsageDataCollectionService", "DefaultContentService", "GraphStyle", function(Settings, Editor, AuthService, NTN, localStorageService, AuthDataService, jwtHelper, $q, $rootScope, UDC, DefaultContentService, GraphStyle) {
    ...
    return _unbind = function() {},
        _sync_object = !1, _connection = function() {
            var connectedRef;
            ...
            login: _login,
            logout: function() {
                return auth.signout(), _unbind(), _sync_object.$destroy()
            },
...
}), angular.module("neo4jApp.controllers").controller("SysinfoController", ["$scope", "Settings", "ProtocolFactory", "$timeout", function($scope, Settings, ProtocolFactory, $timeout) {
    ...
    return $scope.autoRefresh = !1, $scope.sysinfo = {}, null == (base = $scope.sysinfo).primitives && (base.primitives = {}), null == (base1 = $scope.sysinfo).cache && (base1.cache = {
        ...
        available: !1
    }), null == (base3 = $scope.sysinfo).ha && (base3.ha = {}), $scope.refresh = function() {
        var base4;
        return null == (base4 = $scope.sysinfo).kernel && (base4.kernel = {}), ProtocolFactory.getJmxService().getJmx(["org.neo4j:instance=kernel#0,name=Configuration", "org.neo4j:instance=kernel#0,name=Kernel", "org.neo4j:instance=kernel#0,name=Store file sizes", "org.neo4j:instance=kernel#0,name=Primitive count", "org.neo4j:instance=kernel#0,name=Page cache", "org.neo4j:instance=kernel#0,name=Transactions", "org.neo4j:instance=kernel#0,name=High Availability"]).then(function(response) {
            var a, clusterMember, connectedMemberId, i, j, k, l, len, len1, len2, len3, ma, member, r, ref, ref1, ref2, ref3, ref4, results;
            for (ref = response.data, results = [], i = 0, len = ref.length; len > i; i++)
                if (r = ref[i], "org.neo4j:instance=kernel#0,name=Configuration" === (ref1 = r.name) || "org.neo4j:instance=kernel#0,name=Kernel" === ref1 || "org.neo4j:instance=kernel#0,name=Store file sizes" === ref1) results.push(function() {
                ...
                else if ("org.neo4j:instance=kernel#0,name=Primitive count" === r.name) results.push(function() {
                var j, len1, ref2, results1;
            ...
            else if ("org.neo4j:instance=kernel#0,name=Page cache" === r.name) $scope.sysinfo.cache.available = !0, results.push(function() {
                var j, len1, ref2, results1;
                for (ref2 = r.attributes, results1 = [], j = 0, len1 = ref2.length; len1 > j; j++) a = ref2[j], results1.push($scope.sysinfo.cache[a.name] = a.value);
                return results1
            }());
            else if ("org.neo4j:instance=kernel#0,name=Transactions" === r.name) $scope.sysinfo.tx.available = !0, results.push(function() {
                var j, len1, ref2, results1;
                for (ref2 = r.attributes, results1 = [], j = 0, len1 = ref2.length; len1 > j; j++) a = ref2[j], results1.push($scope.sysinfo.tx[a.name] = a.value);
                return results1
            }());
            else if ("org.neo4j:instance=kernel#0,name=High Availability" === r.name) {
                for ($scope.sysinfo.ha.clustered = !0, ref2 = r.attributes, j = 0, len1 = ref2.length; len1 > j; j++)
                    if (a = ref2[j], "InstancesInCluster" === a.name)
                        for ($scope.sysinfo.ha.ClusterMembers = {}, ref3 = a.value, k = 0, len2 = ref3.length; len2 > k; k++) {
                            for (member = ref3[k], clusterMember = {}, ref4 = member.value, l = 0, len3 = ref4.length; len3 > l; l++) ma = ref4[l], clusterMember[ma.name] = ma.value;
    ...
    return $scope.pinned = !1, $scope.pin = function(frame) {
...
}]), angular.module("neo4jApp.services").factory("Bolt", ["Settings", "AuthDataService", "localStorageService", "$rootScope", "$location", "$q", "Utils", function(Settings, AuthDataService, localStorageService, $rootScope, $location, $q, Utils) {
    var _driver, _errorStatus, arrayIntToString, beginTransaction, bolt, boltPlanToRESTPlan, boltPlanToRESTPlanShared, boltResultToRESTResult, boltStatsToRESTStats, boltTransaction, buildErrorObj, callProcedure, clearConnection, connect, createSession, extractDataForGraphFormat, extractDataForRowsFormat, extractPathForRowsFormat, extractPathsForGraphFormat, getDriverObj, getRESTGraphFromBolt, getRESTMetaFromBolt, getRESTRowsFromBolt, getSocketErrorObj, itemIntToString, jmxResultToRESTResult, metaResultToRESTResult, objIntToString, schemaResultToRESTResult, testConnection, testQuery, transaction, versionResultToRESTResult;
    return bolt = window.neo4j.v1, _driver = null, _errorStatus = null, getDriverObj = function(withoutCredentials) {
        var _m, authData, driver, encrypted, host, password, ref, username;
        return null == withoutCredentials && (withoutCredentials = !1), authData = AuthDataService.getPlainAuthData(), host = Settings.boltHost || $location.host(), encrypted = "https" === $location.protocol() ? !0 : !1, ref = authData ? authData.match(/^([^:]+):(.*)$/) : ["", "", ""], _m = ref[0], username = ref[1], password = ref[2], driver = withoutCredentials ? bolt.driver("bolt://" + host, bolt.auth.basic("", ""), {
            encrypted: encrypted
        }) : bolt.driver("bolt://" + host, bolt.auth.basic(username, password), {
            encrypted: encrypted
        })
    }, testQuery = function(driver) {
        var p, q, session;
        return q = $q.defer(), driver.onError = function(e) {
            return e instanceof Event && "error" === e.type ? q.reject(getSocketErrorObj()) : e.code && e.message ? q.reject(buildErrorObj(e.code, e.message)) : void 0
        }, session = driver.session(), p = session.run("CALL db.labels"), p.then(function(r) {
            return session.close(), q.resolve(r)
        })["catch"](function(e) {
            return session.close(), q.reject(e)
        }), q.promise
    }, testConnection = function(withoutCredentials) {
        var driver, q;
        return null == withoutCredentials && (withoutCredentials = !1), q = $q.defer(), driver = getDriverObj(withoutCredentials), testQuery(driver).then(function(r) {
            return q.resolve(r), _errorStatus = null, driver.close()
        })["catch"](function(e) {
            return q.reject(e), _errorStatus = e, driver.close()
        }), q.promise
    }, connect = function(withoutCredentials) {
        var q;
        return null == withoutCredentials && (withoutCredentials = !1), q = $q.defer(), _driver = getDriverObj(withoutCredentials), testQuery(_driver).then(function(r) {
            return q.resolve(r)
        })["catch"](function(e) {
            return "Neo.ClientError.Security.CredentialsExpired" !== e.fields[0].code && (_driver = null), q.reject(e)
        }), q.promise
    }, clearConnection = function() {
        return null != _driver && _driver.close(), _driver = null
    }, createSession = function() {
        return _driver ? _driver.session() : !1
    }, beginTransaction = function(opts) {
        var parameters, q, ref, ref1, session, statement, tx;
        return q = $q.defer(), statement = (null != (ref = opts[0]) ? ref.statement : void 0) || "", parameters = (null != (ref1 = opts[0]) ? ref1.parameters : void 0) || {}, session = createSession(), session ? (tx = session.beginTransaction(), statement || q.resolve(), statement && tx.run(statement, parameters).then(function(r) {
            return q.resolve(r)
        })["catch"](function(e) {
            return q.reject(e)
        })) : (tx = null, q.reject(getSocketErrorObj())), {
            tx: tx,
            session: session,
            promise: q.promise
        }
        ...
        return statement = (null != (ref = opts[0]) ? ref.statement : void 0) || "", parameters = (null != (ref1 = opts[0]) ? ref1.parameters : void 0) || {}, q = $q.defer(), session = session || createSession(), session ? tx ? (p = tx.run(statement, parameters), p.then(function(r) {
            return tx ? tx.commit().then(function(txr) {
                return session.close(), q.resolve(r)
            })["catch"](function(txe) {
                return session.close(), q.reject(txe)
            }) : (session.close(), q.resolve(r))
        })["catch"](function(e) {
            return session.close(), q.reject(e)
        })) : (p = session.run(statement, parameters), p.then(function(r) {
            return session.close(), q.resolve(r)
        })["catch"](function(e) {
            return session.close(), q.reject(e)
    ...
    }, boltTransaction = function(query, parameters) {
        var statements;
    ...
    }, jmxResultToRESTResult = function(r, whatToGet) {
        var filtered;
        return null == whatToGet && (whatToGet = []), r.records ? (r.records = itemIntToString(r.records), filtered = r.records.filter(function(record) {
            return whatToGet.indexOf(record.get("name")) > -1
        }).map(function(record) {
            var origAttributes;
        ...
        return items = keys.map(function(key) {
            ...
            return item.id = item.identity, item
            ...
            return item.startNode = item.start, item.endNode = item.end, item
        }), {
            nodes: nodes,
            relationships: rels
        }
        ...
        var out;
        return item instanceof bolt.types.Node ? item.properties : item instanceof bolt.types.Relationship ? item.properties : item instanceof bolt.types.Path ? [].concat.apply([], extractPathForRowsFormat(item)) : null === item ? item : Array.isArray(item) ? item.map(function(subitem) {
        ...
        var out;
        return item instanceof bolt.types.Node ? item : item instanceof bolt.types.Relationship ? item : item instanceof bolt.types.Path ? [].concat.apply([], extractPathsForGraphFormat(item)) : null === item ? !1 : Array.isArray(item) ? item.map(function(subitem) {
    ...
    }, itemIntToString = function(item) {
        var ref;
        return Array.isArray(item) ? arrayIntToString(item) : "number" == (ref = typeof item) || "string" === ref || "boolean" === ref ? item : null === item ? item : bolt.isInt(item) ? item.toString() : "object" == typeof item ? objIntToString(item) : void 0
    }, arrayIntToString = function(arr) {
        return arr.map(function(item) {
            return itemIntToString(item)
        })
    }, objIntToString = function(obj) {
        return Object.keys(obj).forEach(function(key) {
            return obj[key] = itemIntToString(obj[key])
        }), obj
        ...
        var newStats, stats;
        return summary && summary.updateStatistics ? (stats = summary.updateStatistics._stats, newStats = {}, Object.keys(stats).forEach(function(key) {
            var newKey;
            return newKey = key.replace(/([A-Z]+)/, function(m) {
                return "_" + m.toLowerCase()
            }), newStats[newKey] = stats[key]
        }), newStats.contains_updates = summary.updateStatistics.containsUpdates(), newStats) : {}
...
addLeadingZero = function(num) {
    ...
    var DefaultContent, basic_scripts, example_graphs, profile_scripts, system_scripts;
    return basic_scripts = [{
        ...
        return DefaultContent.prototype.getDefaultDocuments = function() {
            return basic_scripts.concat(example_graphs.concat(profile_scripts.concat(system_scripts)))
...
}]), angular.module("neo4jApp.services").factory("CypherTransactionREST", ["$q", "CypherResult", "Server", "UsageDataCollectionService", "Timer", function($q, CypherResult, Server, UDC, Timer) {
    ...
    return parseId = function(resource) {
        ...
        return CypherTransactionREST.prototype._requestDone = function(promise) {
        ...
        }, CypherTransactionREST.prototype.commit = function(query) {
            ...
            return q = $q.defer(), this.id ? (server_promise = Server.transaction({
                method: "DELETE",
                path: "/" + this.id,
                statements: []
...
}]), angular.module("neo4jApp.services").factory("CypherTransactionBolt", ["$q", "CypherResult", "Bolt", "UsageDataCollectionService", "Timer", "$rootScope", function($q, CypherResult, Bolt, UDC, Timer, $rootScope) {
    ...
    return parseId = function(resource) {
        ...
        var q;
        return q = $q.defer(), promise.then(function(_this) {
            return function(res) {
        ...
        function CypherTransactionBolt() {
            var delegate;
            this._reset(), this.requests = [], delegate = null, this.session = null, this.tx = null
        }
        return CypherTransactionBolt.prototype._requestDone = function(promise) {
            ...
            return this.tx = null, this.session = null
        }, CypherTransactionBolt.prototype.begin = function() {
            var q;
            return q = $q.defer(), q.resolve(), q.promise
        }, CypherTransactionBolt.prototype.commit = function(query) {
            var promise, q, ref, res, session, statements, that, timer, tx;
            return that = this, statements = query ? [{
                statement: query
            }] : [], UDC.increment("cypher_attempts"), q = $q.defer(), ref = Bolt.transaction(statements, this.session, this.tx), tx = ref.tx, promise = ref.promise, session = ref.session, this.tx = tx, this.session = session, timer = Timer.start(), promise.then(function(r) {
                return $rootScope.bolt_connection_failure = !1, r.responseTime = timer.stop().time(), q.resolve({
                    original: r,
                    remapped: Bolt.constructResult(r)
                }), that._reset()
            })["catch"](function(r) {
                var errObj;
                return errObj = Bolt.constructResult(r), ("Socket.Error" === errObj.data.errors[0].code || 0 === errObj.data.errors[0].message.indexOf("WebSocket connection failure")) && ($rootScope.bolt_connection_failure = !0), r.responseTime = timer.stop().time(), q.reject({
                    original: r,
                    remapped: errObj
                }), that._reset()
...
}]), angular.module("neo4jApp.services").factory("UtilityBolt", ["Bolt", "Settings", "$q", "$rootScope", function(Bolt, Settings, $q, $rootScope) {
    return {
        clearConnection: function() {
        ...
        getVersion: function() {
            var q;
            return q = $q.defer(), Bolt.boltTransaction("CALL dbms.components()").promise.then(function(r) {
        ...
        getJmx: function(whatToGet) {
            var name, q;
            return null == whatToGet && (whatToGet = []), q = $q.defer(), name = 1 === whatToGet.length ? whatToGet[0] : "*:*", Bolt.boltTransaction("CALL dbms.queryJmx('" + name + "')").promise.then(function(r) {
                return q.resolve(Bolt.constructJmxResult(r, whatToGet))
            })["catch"](function(e) {
                return q.reject(Bolt.constructResult(e))
            }), q.promise
        },
        makeRequest: function(withoutCredentials, retainConnection) {
            var q, r;
            return q = $q.defer(), r = Bolt.testConnection(withoutCredentials), r.then(function(r) {
                var res;
                return res = Bolt.constructResult(r), retainConnection && Bolt.connect(), res.data.errors.length ? q.reject({
                    status: 401,
                    data: res
                }) : ($rootScope.bolt_connection_failure = !1, q.resolve({}))
            })["catch"](function(err) {
                var errObj;
                return errObj = Bolt.constructResult(err), "Neo.ClientError.Security.CredentialsExpired" === errObj.data.errors[0].code ? (errObj.data.password_change = "true", errObj.status = 403, retainConnection && Bolt.connect()) : "Socket.Error" === errObj.data.errors[0].code || 0 === errObj.data.errors[0].message.indexOf("WebSocket connection failure") ? (errObj.status = 0, $rootScope.bolt_connection_failure = !0) : errObj.status = 401, q.reject(errObj)
            }), q.promise
        },
        setNewPassword: function(username, newPasswd) {
            var q;
            return q = $q.defer(), Bolt.boltTransaction("CALL dbms.changePassword({password})", {
                password: newPasswd
...
}]), angular.module("neo4jApp.services").factory("UtilityREST", ["Server", "Settings", "$q", function(Server, Settings, $q) {
    return {
        clearConnection: function() {
            return angular.noop
        },
    ...
    return {
        ...
        getAuthService: function(useBolt) {
            return null == useBolt && (useBolt = Settings.useBolt), useBolt ? UtilityBolt : UtilityREST
        },
        getMetaService: function(useBolt) {
            return null == useBolt && (useBolt = Settings.useBolt), useBolt ? UtilityBolt : UtilityREST
        },
        getSchemaService: function(useBolt) {
            return null == useBolt && (useBolt = Settings.useBolt), useBolt ? UtilityBolt : UtilityREST
        },
        getJmxService: function(useBolt) {
            return null == useBolt && (useBolt = Settings.useBolt), useBolt ? UtilityBolt : UtilityREST
        },
        getVersionService: function(useBolt) {
            return null == useBolt && (useBolt = Settings.useBolt), useBolt ? UtilityBolt : UtilityREST
        }
    }
...
"undefined" != typeof global && null !== global && (global.neo = global.neo || {}), "undefined" != typeof window && null !== window && (window.neo = window.neo || {}), neo = ("undefined" != typeof global && null !== global ? global.neo : void 0) || ("undefined" != typeof window && null !== window ? window.neo : void 0), neo.serializer = function() {
    function serializer(opts) {
        null == opts && (opts = {}), this.options = (new neo.helpers).extend(opts, {
            ...
            return null == string ? "" : ("string" != typeof string && (string = JSON.stringify(string)), string.length ? ((string.indexOf(this.options.delimiter) > 0 || string.indexOf('"') >= 0) && (string = '"' + string.replace(/"/g, '""') + '"'), string) : '""')
        }
    }
    return serializer
}();
